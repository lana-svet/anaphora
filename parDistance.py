...
with open('ana_test.pairs', encoding="utf8") as fd:  # списки первых и вторых цифр из пар
    for line in fd: 
        match = pattern.search(line)
        first_num.append(match.group(1))            
        second_num.append(match.group(2))           
       
with open('ana_test.groups', encoding="utf8") as d:  # словарь вида ИГ - вершина
    for s in d:
        s = s.strip().split('\t')
...
with open('ana_test.tab', encoding="utf8") as doc:   # список тэгов и номеров токенов
    for r in doc:
        r = r.strip().split('\t')

...
if not has_marks:
    editor.db_cursor.execute("ALTER TABLE syntax_groups ADD marks ENUM('bad', 'suspicious', 'no head')")
...
# в группах типа "перечисление" убрать вершины, поставить тэг "нет вершины"
...
for row in lists:
    editor.db_cursor.execute("UPDATE syntax_groups SET head_id = 0, marks = 'no head' WHERE group_id = " + str(row['parent_gid']))       
...
for row in supplement:
    editor.db_cursor.execute("SELECT MIN(child_gid) as first_group FROM syntax_groups_complex WHERE parent_gid = " + str(row['parent_gid']))
